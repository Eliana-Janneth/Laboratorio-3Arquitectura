  ...
  return (
    <div>
      <h2 id="profesor-heading" data-cy="ProfesorHeading">
        <Translate contentKey="labArquiJHipsterApp.profesor.home.title">Profesors</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="labArquiJHipsterApp.profesor.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/profesor/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="labArquiJHipsterApp.profesor.home.createLabel">Create new Profesor</Translate>
          </Link>
        </div>
      </h2>
        ...
        {profesorList && profesorList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th>
                  <Translate contentKey="labArquiJHipsterApp.profesor.id">ID</Translate>
                </th>
                <th>
            ...
            <tbody>
              {profesorList.map((profesor, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{profesor.usuarioPortal}</td>
                  <td>{profesor.grupos ? <Link to={`/grupo/${profesor.grupos.id}`}>{profesor.grupos.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="labArquiJHipsterApp.profesor.home.notFound">No Profesors found</Translate>
            </div>
          )
        )}
      </div>
