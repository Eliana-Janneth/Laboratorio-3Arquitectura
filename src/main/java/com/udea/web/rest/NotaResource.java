...
/**
 * REST controller for managing {@link com.udea.domain.Nota}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class NotaResource {
    ...
    public ResponseEntity<Nota> partialUpdateNota(
        ...
        Optional<Nota> result = notaRepository
            .findById(nota.getId())
            .map(existingNota -> {
                if (nota.getNotaId() != null) {
                    existingNota.setNotaId(nota.getNotaId());
                }
                if (nota.getNota() != null) {
                    existingNota.setNota(nota.getNota());
                }
                if (nota.getFechaCreacion() != null) {
                    existingNota.setFechaCreacion(nota.getFechaCreacion());
                }
