...
/**
 * REST controller for managing {@link com.udea.domain.Curso}.
 */
@RestController
@RequestMapping("/api")
@Transactional
        ...
        Optional<Curso> result = cursoRepository
            ...
            .map(existingCurso -> {
                if (curso.getCursoId() != null) {
                    existingCurso.setCursoId(curso.getCursoId());
                }
                ...
                if (curso.getCredito() != null) {
                    existingCurso.setCredito(curso.getCredito());
                }
